...
export default function TagEditModal({ ticket, tag, onClose, onUpdate }) {
    if (!ticket || !tag) return null; // Prevent crash if props are missing
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            // Safe Map: Check if t exists
            const newTags = currentTags.map(t => (t && t.id === tag.id ? updatedTag : t));

            await ticketService.updateTicket(ticket.id, { tags: newTags });
            toast.success("Etiqueta actualizada");
            if (onUpdate) onUpdate({ ...ticket, tags: newTags });
            onClose();
        } catch (error) {
            console.error(error);
    ...
    const handleDelete = async () => {
        ...
        try {
            const currentTags = Array.isArray(ticket.tags) ? ticket.tags : [];
            // Safe Filter: Check if t exists
            const newTags = currentTags.filter(t => t && t.id !== tag.id);

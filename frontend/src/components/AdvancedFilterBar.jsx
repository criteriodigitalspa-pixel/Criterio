...
export default function AdvancedFilterBar({
    ...
    savedViews         // [NEW] Saved Views Slot
}) {
    const {
    ...
    return (
        <div id="advanced-filter-bar" className="bg-[#0f172a] border-b border-gray-800 sticky top-0 z-50 shadow-lg h-14 flex items-center relative">
            <div className="w-full px-4 flex items-center gap-4 max-w-[2600px] mx-auto">
                ...
                {/* NEW: SAVED VIEWS */}
                {savedViews && <div className="ml-2 shrink-0">{savedViews}</div>}

                {/* 3. FILTER MENU (Moved Here) */}
                <div className="relative" ref={popoverRef}>
                    ...
                    {isExpanded && (
                        <div
                            className="absolute top-[120%] left-0 w-[800px] bg-[#1e293b]/95 backdrop-blur-xl border border-gray-700 rounded-xl shadow-2xl p-5 z-[100] animate-in fade-in zoom-in-95 origin-top-left ring-1 ring-black/50 overflow-visible"
                        >
                            {/* ACTIONS HEADER */}
                ...
                {/* 4. SPACER (Replaces Chips) */}
                <div className="flex-1"></div>

                {/* 5. METRICS & ACTIONS (Right) */}

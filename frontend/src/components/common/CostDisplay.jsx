...
const CostDisplay = ({ ticket, compact = false }) => {
    ...
    const {
        totalCost, ramDelta, diskDelta, isModified, baseCost,
        serviceCost, sparePartsCost, extraCosts, viaticoCost, publicidadCost,
                ...
                {showBreakdown && (
                    <div className="absolute top-0 right-7 mt-0 w-72 bg-[#0f172a]/95 backdrop-blur-md rounded-xl border border-gray-700 shadow-2xl p-4 z-[9999] animate-in fade-in zoom-in-95 origin-top-right text-left ring-1 ring-white/10">
                        ...
                        <div className="space-y-1.5">
                            ...
                            {diskDelta !== 0 && (
                                <div className="flex justify-between items-center text-xs">
                                    <span className={clsx(diskDelta > 0 ? "text-purple-400" : "text-emerald-400")}>
                                        {diskDelta > 0 ? 'Upgrade Disco' : 'Ahorro Disco'}
                                    </span>
                                    <span className={clsx("font-mono font-bold", diskDelta > 0 ? "text-red-400" : "text-green-400")}>
                                        {diskDelta > 0 ? '+' : ''}{format(diskDelta)}
                                    </span>
                                </div>
                            )}

                            {/* Reparacion */}
                            {(sparePartsCost > 0 || serviceCost > 0) && (
                                <div className="flex justify-between items-center text-xs">
                                    <span className="text-orange-400">Reparación</span>
                                    <span className="text-red-400 font-mono font-bold">+{format(sparePartsCost + serviceCost)}</span>
                                </div>
                            )}

                            {/* Viatico + Publicidad + Extras */}
                            {(extraCosts > 0 || viaticoCost > 0 || publicidadCost > 0) && (
                                <div className="flex justify-between items-center text-xs">
                                    <span className="text-yellow-400">Extras/Logística</span>
                                    <span className="text-red-400 font-mono font-bold">+{format(extraCosts + viaticoCost + publicidadCost)}</span>
                                </div>
                            )}
    ...
    return (
        <div className="bg-gray-900/50 p-4 rounded-xl border border-gray-700 space-y-3">
            ...
            <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-700/50">
                ...
                {diskDelta !== 0 && (
                    <span className={clsx("text-[10px] flex items-center gap-1 px-2 py-1 rounded border border-gray-700 bg-gray-800", diskDelta > 0 ? "text-purple-400" : "text-emerald-400")}>
                        {diskDelta > 0 ? 'Disco' : 'Ahorro Disco'}: {diskDelta > 0 ? '+' : ''}{format(diskDelta)}
                    </span>
                )}
                ...
                {viaticoCost > 0 && (
                    <span className="text-[10px] text-pink-300 bg-pink-900/20 px-2 py-1 rounded border border-pink-900/30">
                        Viático: +{format(viaticoCost)}
                    </span>
                )}
                {publicidadCost > 0 && (
                    <span className="text-[10px] text-indigo-300 bg-indigo-900/20 px-2 py-1 rounded border border-indigo-900/30">
                        Publicidad: +{format(publicidadCost)}
                    </span>
                )}
            </div>
        </div>

...
export default function ProductModal({ isOpen, onClose, checkProduct, onSave }) {
    ...
    useEffect(() => {
        if (checkProduct) {
            ...
            setFormData({
                name: '', sku: '', category: '', quantity: 0, cost: 0, price: 0, minStock: 5, description: '',
    ...
    const handleChange = (e) => {
        ...
        setFormData(prev => ({
            ...prev,
            [name]: ['quantity', 'price', 'cost', 'minStock'].includes(name) ? parseFloat(value) : value
        }));
    };
            ...
            <div className="w-full max-w-3xl rounded-2xl bg-gray-800 shadow-2xl border border-gray-700 max-h-[90vh] overflow-y-auto custom-scrollbar">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        <div>
                            <label className="mb-2 block text-xs font-bold text-red-400 uppercase">Costo Base ($)</label>
                            <input type="number" name="cost" value={formData.cost || 0} onChange={handleChange} className={getInputClass()} required min="0" step="0.01" />
                        </div>

                        <div>
                            <label className="mb-2 block text-xs font-bold text-green-400 uppercase">Precio Venta ($)</label>

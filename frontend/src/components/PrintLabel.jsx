...
export default function PrintLabel({ ticket, id, config = {}, containerRotation = 0 }) {
    ...
    const formatRam = () => {
        const details = Array.isArray(ticket.ram) ? ticket.ram : (ticket.ram?.detalles || []);
        if (!details.length) return '-';
        let total = 0;
        const parts = details.map(d => {
            const val = parseInt(d);
            if (!isNaN(val)) total += val;
            return d.toString().toUpperCase().includes('GB') ? d : `${d}GB`;
        });
        const joined = parts.join(' + ');
        return total > 0 ? `${joined} (${total}GB)` : joined;
    };
    const ram = formatRam();

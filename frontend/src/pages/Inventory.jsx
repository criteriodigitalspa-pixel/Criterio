                ...
                {!loading && (
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                        <div className="bg-gray-800 p-4 rounded-2xl border border-gray-700 shadow-lg">
                            <h3 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">Total Inventario</h3>
                            <div className="text-2xl font-black text-white flex items-end gap-2">
                                {products.reduce((sum, p) => sum + p.quantity, 0)} <span className="text-sm font-medium text-gray-400 mb-1">unidades</span>
                            </div>
                            <div className="text-xs text-gray-500 mt-1">{products.length} SKUs únicos</div>
                        </div>

                        <div className="bg-gray-800 p-4 rounded-2xl border border-gray-700 shadow-lg">
                            <h3 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">Valoración (Costo)</h3>
                            <div className="text-2xl font-black text-blue-400">
                                ${products.reduce((sum, p) => sum + (p.quantity * (p.cost || 0)), 0).toLocaleString()}
                            </div>
                            <div className="text-xs text-gray-500 mt-1">Capital invertido en stock</div>
                        </div>

                        <div className="bg-gray-800 p-4 rounded-2xl border border-gray-700 shadow-lg">
                            <h3 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">Potencial Venta</h3>
                            <div className="text-2xl font-black text-green-400">
                                ${products.reduce((sum, p) => sum + (p.quantity * (p.price || 0)), 0).toLocaleString()}
                            </div>
                            <div className="text-xs text-gray-500 mt-1">Ganancia Estimada: <span className="text-green-300 font-bold">+{Math.round(((products.reduce((sum, p) => sum + (p.quantity * p.price), 0) - products.reduce((sum, p) => sum + (p.quantity * (p.cost || 0)), 0)) / (products.reduce((sum, p) => sum + (p.quantity * p.price), 0) || 1)) * 100)}%</span></div>
                        </div>

                        <div className="bg-gray-800 p-4 rounded-2xl border border-gray-700 shadow-lg relative overflow-hidden group">
                            <div className="absolute right-0 top-0 p-3 opacity-10 group-hover:opacity-20 transition-opacity">
                                <AlertTriangle className="w-16 h-16 text-red-500" />
                            </div>
                            <h3 className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">Stock Crítico</h3>
                            <div className="text-2xl font-black text-red-400">
                                {products.filter(p => p.quantity <= (p.minStock || 5)).length} <span className="text-sm font-medium text-gray-400 mb-1">SKUs</span>
                            </div>
                            <div className="text-xs text-red-400/70 mt-1 font-bold">Requieren reabastecimiento</div>
                        </div>
                    </div>
                )}

...
import { X, Save, AlertTriangle, Check, Smartphone, Cpu, CircuitBoard, HardDrive, Monitor, Battery, Hash, RotateCcw, ArrowRight, AlertCircle, RefreshCw, CheckCircle, Info, CreditCard, Zap, Printer, Wand2 } from 'lucide-react';
...
export default function AdditionalInfoModal({ ticket, onClose, onUpdate }) {
    ...
    // Customize initialization to include conFactura
    ...
    const [ram, setRam] = useState(ticket.ram || { slots: 2, detalles: [] });
    ...
    // ... (lines 35-176 are unchanged until handleSave)
    ...
    const handleSave = async (isAuto = false) => {
        ...
        try {
            ...
            await ticketService.updateTicketAttributes(ticket.id, updates, {
                userId: user.uid,
                reason: 'Actualización Ficha Técnica',
                note: isComplete ? 'Ficha Técnica Completada' : 'Ficha Técnica Guardada (Parcial)'
            });
            toast.success('Información técnica guardada');
                            ...
                            <button
                                ...
                                onClick={async () => {
                                    ...
                                    try {
                                        ...
                                        // 2. Get Apps
                                        const compatibleApps = descriptionGenerator.getCompatibleApps(scores);

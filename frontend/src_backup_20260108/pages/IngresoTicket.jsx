...
import { Save, Printer, Package, DollarSign, Database, Monitor, Disc } from 'lucide-react';
...
import { CheckSquare, Users, ShoppingBag, Cpu } from 'lucide-react';
...
export default function IngresoTicket() {
        ...
        try {
            if (lastCreatedBatch.length > 0) {
                ...
                for (let i = 0; i < lastCreatedBatch.length; i++) {
                    await printService.printLabel(lastCreatedBatch[i], 'initial');
                    // Small throttle to avoid Firestore rate limits or Agent flood
                    await new Promise(r => setTimeout(r, 300));
                }
            } else if (lastCreatedTicket) {
                await printService.printLabel(lastCreatedTicket, 'initial');
            }
    ...
    const [formData, setFormData] = useState({
        ...
        // Advanced Logic
        ram: { slots: 0, detalles: [] },
        disco: { slots: 0, detalles: [] } // User requested default 0, manual selection required
    });
    ...
    const handleSaveBatch = async () => {
        ...
        runBackgroundTask(`Ingresando Lote(${batchList.length} equipos)...`, async (updateProgress) => {
            ...
            const ticketsToSave = batchList.map(item => {
                const { tempId, cpuBrand, cpuGen, ...ticketData } = item;
                return {
                    ...ticketData,
                    additionalInfo: {
                        cpuBrand: cpuBrand,
            ...
            for (let i = 0; i < total; i++) {
                await printService.printLabel(tickets[i], 'initial');
            ...
            // 4. Download PDF/ZIP
            updateProgress(95);
            // This relies on DOM elements existing or logic being pure.
            // printService.downloadBatchZip uses 'document.getElementById'. 
            // THIS WILL FAIL IF WE NAVIGATE AWAY.
            // FIX: We need a purely generative approach or we accept we can't download ZIP in background easily if views are gone.
            // Wait, printService.downloadBatchZip uses `html2canvas` on a rendered component.
            // If we are on KanbanBoard, `PrintLabelInitial` for these tickets IS NOT RENDERED.
            // CRITICAL: We cannot use `html2canvas` if the elements aren't in the DOM.
            // WE MUST use `jsPDF` raw generation (text/lines) OR `PrintLabelInitial` must be mounted somewhere in the background.
            // Strategy: We will SKIP the ZIP download for now in background to avoid crash, OR we strictly assume user stays?
            // User asked for "transfiera automaticamente al tablero".
            // So we are NOT on the page.

            // ALTERNATIVE: Print Agent is handling the "Print".
            // For the PDF Download... maybe we don't do it automatically if it's too risky, or we use a "Hidden Renderer" in the Layout.
            // Let's rely on the Print Agent for the PHYSICAL print (Automatic).
            // For PDF, let's skip automatic download to avoid the HTML2Canvas dependency issue, or warn user.
            // Actually, `IngresoTicket` had specific logic for zip.
            // Let's try to notify user that Print was sent. Automatic Download might be overkill if they have the Print Agent.
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            try {
                await printTask(newTicket);
            } catch (printErr) {
                console.error("Print Error (Non-blocking):", printErr);
                toast.error("Ticket creado, pero error al imprimir.");
            }

            toast.success("Ticket Ingresado Exitosamente");

            // RESET & NAVIGATE
            // Force reset state before unmount to be safe? 
            // Actually, unmount happens on navigate. 
            // But if user hits back, we want it clean.
            resetForm();
            navigate('/tickets');
                    ...
                    */}
                                    ...
                                    {Array.from({ length: formData.disco.slots }).map((_, i) => (
                                        <div key={i}>
                                            <input
                                                ...
                                                className="w-full bg-gray-800 border border-gray-600 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-all"
                                                placeholder={`Disco ${i + 1} (Escriba o seleccione)...`}
                                            />
                                            <datalist id={`disk - options - ${i} `}>
                    ...
                    <div className="flex flex-col md:flex-row gap-6 items-end">
                        ...
                        <div className="flex-1 w-full relative">
                            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-2">
                                <ShoppingBag className="w-4 h-4" /> Proveedor (Opcional)
                            </label>
                            <input
                                type="text" name="proveedor"
                                placeholder="Ej: Importadora X"
                                value={formData.proveedor || ''} onChange={(e) => handleChange({ target: { name: 'proveedor', value: e.target.value.toUpperCase() } })}
                                className="w-full bg-gray-900/50 border border-gray-700 rounded-xl px-4 py-4 text-white placeholder-gray-600 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all font-medium"
                            />
                        </div>
                    </div>
                    ...
                    <div className="pb-4">
                        <label className="inline-flex items-center cursor-pointer select-none group">
                            <input
                                type="checkbox" name="conFactura"
                                checked={formData.conFactura} onChange={handleChange}
                                className="rounded bg-gray-700 border-gray-600 text-blue-500 focus:ring-blue-500 h-6 w-6 transition-all"
                            />
                            <span className="ml-3 text-gray-300 font-medium group-hover:text-white transition-colors">¿Factura?</span>
                        </label>
                    </div>

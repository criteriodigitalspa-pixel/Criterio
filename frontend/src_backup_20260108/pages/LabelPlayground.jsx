...
export default function LabelPlayground() {
    ...
    // Load from Firestore + LocalStorage Fallback
    useEffect(() => {
        const fetchTemplates = async () => {
            ...
            try {
                ...
                querySnapshot.forEach((doc) => {
                    let data = doc.data();
                    // Fix/Migration: Small labels legacy fix
                ...
                console.warn("Could not load from Firestore (Permissions/Offline):", error);
                source = 'local';
            }

            // Fallback to LocalStorage if DB failed or was empty
            if (loaded.length === 0) {
                ...
                if (local) {
                    try {
                        ...
                        if (source === 'local') toast("⚠️ Modo Offline: Cargado desde Local", { icon: '📂' });
                    } catch (e) { console.error("Bad LocalStorage", e); }
                }
            }
            ...
            setTemplates(loaded);
            if (loaded.length > 0) setCurrentTemplateId(loaded[0].id);
            setLoading(false);
        };

        fetchTemplates();

        // Timeout removed as logic handles errors gracefully now
    }, []);
    ...
    const handleSave = async () => {
        ...
        if (!auth.currentUser) {
            toast("💾 Guardado LOCALMENTE (Sin sesión iniciada)", { icon: '💻' });
            return;
        }
        ...
        try {
            await setDoc(doc(db, 'label_templates', currentTemplate.id), {
                ...
                modifiedByEmail: auth.currentUser.email
            });
    ...
    const handleTest = async (action) => {
        ...
        try {
            ...
            // SMART ROTATION FIX:
            // The user correctly points out that PDF (0°) and Print (90°) are contradictory but required.
            // For Detailed labels (which are 70x50), we MUST print them at 90° (50x70) to fit on the 50mm roll.
            // We automate this override so the user can keep the UI at 0°.
            if (isDetailed && action === 'PRINT') {
                activeRotation = 90;
            }
                ...
                <div className="p-4 bg-gray-900 border-b border-gray-700 flex justify-between items-center relative">
                    ...
                    <div className="flex gap-2 items-center">
                        <select
                            ...
                            className="bg-gray-800 text-white text-[10px] p-1 rounded border border-gray-600 focus:outline-none"
                            title="Forzar Tamaño de Papel"
                        >
            ...
            <div className="absolute top-0 left-0 pointer-events-none opacity-0" style={{ transform: 'translateX(-9999px)' }}>
                {currentTemplate && currentTemplate.type === 'detailed' && (
                    <>
                        ...
                        <div id="preview-target-detailed-print" className="inline-block bg-white">
                            <PrintLabel
                                ticket={mockTicket}
                                id="print-full-print"
                                config={currentTemplate.config}
                        ...
                        <div id="preview-target-detailed-pdf" className="inline-block bg-white">
                            <PrintLabel
                                ticket={mockTicket}
                                id="print-full-pdf"
                                config={currentTemplate.config}
                ...
                {currentTemplate && currentTemplate.type === 'simple' && (
                    <>
                        {/* PRINT VERSION */}
                        <div id="preview-target-simple-print" className="inline-block bg-white">
                            <PrintLabelInitial
                                ticket={mockTicket}
                                show={false}
                                config={currentTemplate.config}
                        ...
                        {/* PDF VERSION */}
                        <div id="preview-target-simple-pdf" className="inline-block bg-white">
                            <PrintLabelInitial
                                ticket={mockTicket}
                                show={false}
                                config={currentTemplate.config}
